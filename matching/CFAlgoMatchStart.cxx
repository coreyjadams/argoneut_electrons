...
//----------------------------------------------------------------------------------------------
{
  ...
  int n_hits = 15;
  std::vector<larutil::Hit2D> close_hits = GetClosestHits(shower, start_point, n_hits, true);
...
std::vector<larutil::Hit2D> CFAlgoMatchStart::GetClosestHits(const cluster::cluster_params & params,
    larutil::Point2D start,
    int n_hits, bool forward_only) {
  auto all_hits = params.hit_vector;

  std::vector<larutil::Hit2D> close_hits;
