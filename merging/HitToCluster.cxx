...
bool HitToCluster::analyze(storage_manager* storage) {
  ...
  for (unsigned int i = 0; i < ass_info.size(); i ++) {
    ...
    if ( ! out_cluster_v -> back().IsMergedCluster()) {
      ...
      out_cluster_v -> back().set_original_producer(_input_producer);
  ...
  int clust_id = out_cluster_v->back().ID() + 1;
  ...
  for (auto index : leftoverHitIndices) {
    ...
    if ( ! out_cluster_v -> back().IsMergedCluster()) {
      out_cluster_v -> back().set_is_merged(true);
      out_cluster_v -> back().set_original_producer(_input_producer);
      out_cluster_v -> back().set_original_indexes(null);
    }
  ...
  // std::cout << "original_hit_ass size is " << ass_info.size() << std::endl;
