  ...
  const ::cluster::cluster_params &cluster2)
{
  ...
  const ::cluster::cluster_params * big;
  ...
  for (unsigned int i = 0; i < big->PolyObject.Size(); ++i) {

    float w0 = big->PolyObject.Point(i).first;
    float t0 = big->PolyObject.Point(i).second;
  ...
  for (unsigned int i = 0; i < small->PolyObject.Size(); ++i) {

    float w1 = small->PolyObject.Point(i).first;
    float t1 = small->PolyObject.Point(i).second;
...
bool CBAlgoMergeWithinBoundary::isBig(const ::cluster::cluster_params &cluster) {
...
bool CBAlgoMergeWithinBoundary::isSmall(const ::cluster::cluster_params &cluster)
{

  size_t N_Hits = cluster.hit_vector.size();
  double length = cluster.length;
  float ep = cluster.eigenvalue_principal;
...
// float* CBAlgoMergeWithinBoundary::inBoundary(const ::cluster::cluster_params &cluster){
...
//  for (unsigned int i = 0; i < cluster.PolyObject.Size(); ++i) {

//    float w = cluster.PolyObject.Point(i).first;
//     float t = cluster.PolyObject.Point(i).second;
