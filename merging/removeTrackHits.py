...
def main(**args):
    ...
    if args['data_output'] == None:
        ...
        if args['verbose']:
            print "No event output file selected.  \
                  If necessary, output will go to:"
            print "\t"+args['data_output']

    if args['ana_output'] == None:
        args['ana_output'] = "default_ana_output.root"
        if args['verbose']:
            print "No ana output file selected.  \
                  If necessary, output will go to:"
    ...
    rth.SetOutputProducer("ccNoTracks2")
    my_proc.add_process(rth)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-o", "--data-output",
                        help="Output data file, if event is changed")
    parser.add_argument("-a", "--ana-output",
