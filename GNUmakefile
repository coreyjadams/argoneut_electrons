...
#
SUBDIRS := minos #ADD_NEW_SUBDIR ... do not remove this comment from this line
...
all:
	@for i in $(SUBDIRS); do ( echo "" && echo "Compiling $$i..." && cd $(LARLITE_USERDEVDIR)/argoneut_electrons2/$$i && $(MAKE) ) || exit $$?; done
#####################################################################################
#
# CLEANs...
#
clean:
	@for i in $(SUBDIRS); do ( echo "" && echo "Cleaning $$i..." && cd $(LARLITE_USERDEVDIR)/argoneut_electrons2/$$i && $(MAKE) clean && rm -f $(LARLITE_LIBDIR)/$$i.* ) || exit $$?; done
...
doxygen:
	...
	@mkdir -p $(LARLITE_USERDEVDIR)/argoneut_electrons2/doc/dOxygenMyProject
	@doxygen $(LARLITE_USERDEVDIR)/argoneut_electrons2/doc/doxygenMyProject.script
